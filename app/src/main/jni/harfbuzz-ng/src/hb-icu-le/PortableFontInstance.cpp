...
PortableFontInstance::PortableFontInstance(hb_face_t *face, float xScale, float yScale,
                                           LEErrorCode &status)
        : fFace(face), fXScale(xScale), fYScale(yScale), fUnitsPerEM(0), fAscent(0), fDescent(0),
          fLeading(0),
          fNAMETable(NULL), fNameCount(0), fNameStringOffset(0), fCMAPMapper(NULL),
...
const char *PortableFontInstance::getNameString(le_uint16 nameID, le_uint16 platformID,
                                                le_uint16 encodingID, le_uint16 languageID) const {
    if (fNAMETable == NULL) {
        LETag nameTag = LE_NAME_TABLE_TAG;
        PortableFontInstance *realThis = (PortableFontInstance * )
        this;
    ...
    for (le_int32 i = 0; i < fNameCount; i += 1) {
        ...
        if (SWAPW(nameRecord->platformID) == platformID &&
            SWAPW(nameRecord->encodingID) == encodingID &&
...
const LEUnicode16 *PortableFontInstance::getUnicodeNameString(le_uint16 nameID,
                                                              le_uint16 platformID,
                                                              le_uint16 encodingID,
                                                              le_uint16 languageID) const {
    if (fNAMETable == NULL) {
        LETag nameTag = LE_NAME_TABLE_TAG;
        PortableFontInstance *realThis = (PortableFontInstance * )
        this;
    ...
    for (le_int32 i = 0; i < fNameCount; i += 1) {
        ...
        if (SWAPW(nameRecord->platformID) == platformID &&
            ...
            LEUnicode16 *name = (LEUnicode16 * )(
                    ((char *) fNAMETable) + fNameStringOffset + SWAPW(nameRecord->offset));
...
le_bool PortableFontInstance::getGlyphPoint(LEGlyphID /*glyph*/, le_int32 /*pointNumber*/,
                                            LEPoint &/*point*/) const {
    return FALSE;
}
...
LEGlyphID PortableFontInstance::mapCharToGlyph(LEUnicode32 ch, const LECharMapper *mapper,
